        ...
        preferencesAPI.savePreferences(loggedInUser.getUsername(), clickedInstruments).enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(@NonNull Call<ResponseBody> call, @NonNull Response<ResponseBody> response) {
                if (response.isSuccessful()) {
                    Intent userProfileIntent = new Intent(PreferencesActivity.this, StudentHomeActivity.class);
                    userProfileIntent.putExtra("loggedInUser", loggedInUser);
                    startActivity(userProfileIntent);
                } else {
                    Toast.makeText(PreferencesActivity.this, "Neuspesno cuvanje preferencija!", Toast.LENGTH_SHORT).show();
                }
            }
